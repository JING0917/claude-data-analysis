...
--******************************************************************--
--author: dahe
--create time: 2025-02-20 15:07:54
--******************************************************************--
-- 霸王餐最近完单距今天数
...
from
    ...
    (select
        ...
        ,real_rebate_amt
    from dwd.dwd_sr_silkworm_explore_order
    where dt between date_sub(current_date(),interval 180 day) and date_sub(current_date(),interval 1 day) -- 此为示例，需根据触达成功时间来筛选
            and status in (5,19,11,14,17,18,22,23,28)
    ) a
left join
        -- 活动
        ...
        where dt between '2024-06-01' and date_sub(current_date(),interval 1 day) -- 此为示例，需根据触达成功时间来筛选，绝大部分活动是当日内生效，鲜有跨天活动
...
-- 霸王餐召回&探店召回&砍价转化
select 
    count(distinct a.user_id) as unum
...
-- 霸王餐召回&探店召回&砍价非转化
select 
    count(distinct if(c.user_id is null,a.user_id,null)) as unum
from bwc_zh_user a inner join exp_zh_user b on a.user_id=b.user_id
...
-- 霸王餐召回&探店非召回&砍价转化
select 
    count(distinct if(b.user_id is null,a.user_id,null)) as unum
...
-- 霸王餐召回&探店非召回&砍价非转化
select 
    count(distinct if(b.user_id is null and c.user_id is null,a.user_id,null)) as unum
from bwc_zh_user a left join exp_zh_user b on a.user_id=b.user_id
left join kj_zh_user c on a.user_id=c.user_id
;
...
select 
    count(distinct a.user_id) as unum
from un_bwc_zh_user a inner join un_exp_zh_user b on a.user_id=b.user_id
inner join un_kj_zh_user c on a.user_id=c.user_id
;