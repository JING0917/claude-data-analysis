...
CREATE TABLE if not exists dws.dws_sr_traffic_homepage_mix_expflow_d (
  ...
  activity_status varchar(50) not null comment '活动状态',
...
ENGINE=OLAP
PRIMARY KEY (statistics_date,county_id,platform_name,app_version,expno,expflow,activity_type,promotion_id)
COMMENT "首页融合归因实验日数据"
...
CREATE VIEW IF NOT EXISTS origin_traffic_info (time,event,county_id,activity_id,uid,platform_name,app_version,activity_type,user_id,expno,expflow) AS
  (SELECT time,
          event,
          ...
          '0' as expno,
          if(right(distinct_id,1) between 0 and 4,'实验组','对照组') expflow
...
CREATE VIEW IF NOT EXISTS traffic_info (time,event,county_id,activity_id,uid,platform_name,app_version,activity_type,user_id,expno,expflow) AS
  (SELECT time,
          event,
          CASE
...
CREATE VIEW IF NOT EXISTS feed_info (statistics_date,county_id,activity_id,platform_name,app_version,activity_type,expno,expflow,expouse_num,expouse_uids,clc_num,clc_uids) AS
...
CREATE VIEW IF NOT EXISTS agg_funnel (statistics_date,county_id,activity_id,platform_name,app_version,expno,expflow,activity_type,view_uids,pv,bm_uids,bm_clc_num, order_uids,bm_order_num,valid_order_uids,valid_order_num,profit,xx_bm_order_uids,xx_bm_order_num,xx_valid_order_uids,xx_valid_order_num) AS
