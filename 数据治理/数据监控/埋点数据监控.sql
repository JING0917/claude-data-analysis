...
hourly_aggregation AS (
    SELECT
        ...
        -- 搜索关键词缺失统计（仅对搜索事件有效）
        SUM(CASE WHEN event IN ('Search_Click', 'Search_Result_Ex', 'Search_Result_Click')
                 AND (query_word IS NULL OR query_word = '') THEN 1 ELSE 0 END) AS missing_keywords_count,
        SUM(CASE WHEN event IN ('Search_Click', 'Search_Result_Ex', 'Search_Result_Click') THEN 1 ELSE 0 END) AS search_events_total
...
with_comparison AS (
    SELECT
        ...
        -- 搜索关键词缺失统计（直接从聚合结果获取）
        h.missing_keywords_count,
        CASE
            WHEN h.is_search_event AND h.search_events_total > 0 THEN
                ROUND(h.missing_keywords_count * 100.0 / h.search_events_total, 2)
